...
- Gebeurt via shell commands op de achtergrond in lims
- R gaat (doorgaans) NIET zelf resultaten wegschrijven naar Labware LIMS tabellen
    - R schrijft naar custom tabellen C_
    - De gegevens hieruit kunnen de lims-functies dan gebruiken om de officiële tabellen te vullen
    - De lims functies voorzien immers functionaliteit die ingewikkeld kan worden:
        - Er worden veel meer tabellen geaffecteerd
            - De databank is ook niet volledig genormeerd 
...
graph TD
    ...
    %% Door deze pijlen apart te definiëren en 'linkStyle' te gebruiken,
...
qn = qn + ", ps.C_CERTIFIED_VALUE, ps.C_CERTIFIED_SD, ps.C_CTR_X, ps.C_CTR_SD, ps.PRODUCT, ps.VERSION, b.C_DATE_BATCHRUN " , b.C_DATE_BATCHRUN
...
qn = qn + Chr(10) + " inner join batch b on b.NAME = bo.BATCH" 
...
LIMS(["Lims routine"])
RCOMM["Rscript<br/>args: scriptnaam, odbc, call_id, user"]
LOAD["Laden tidyverse en inbolimsintern"]
VGL["Vergelijk bestaande met nieuwe scripts en kopieer indien nodig"]
SETUP["Setup environment"]
VARS[/"conn</br>user_name</br>call_id<br/>params</br>env"/]
STARTLOG["Start logging in C_RSCRIPT_LOG met status P"]
PARAMS["params variabele uitpakken"]
CORE["Voer kernroutine uit"]
FINALIZE["Beëindig log met status C of E"]
REPORTFILES[/"Rapportagefiles (meestal html)"/]
LIMSDB[('Lims DB')]
LIMS_SUB(["Ga verder met LIMS Routine"])
...
- De scripts staan in de inst-map van het package onder lims_scripts
    ...
    - indien anders, worden de bestaande gebackupt en de nieuwe er geplaatst
...
- Alle scripts werken nu op een gestandaardiseerde manier
    - vroeger tekstuele logging
    - vroeger werkte iedere routine een klein beetje anders
...
- derde volledig nieuwe iteratie
    - versie 1: proof of concept
        - alles volledig via files, geen db sturing
    - versie 2:
        - github
        - organische groei
        - alles een beetje anders
        - communicatie via db parameters
        - plain html rapporten
    - versie 3 (vanaf lims 8)
        - omgeving-onafhankelijk: scripts werken op dev, uat en prd
        - logging via db en terugkoppeling naar gebruiker
        - introductie  pandoc, css en javascript
